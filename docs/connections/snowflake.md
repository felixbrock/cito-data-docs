...
6. In case you are not planning to set up the dbt integration with Cito, please hit the 🔄️ (refresh) button to trigger the initial processing of your Snoflake instance, which might take a few minutes. Once the intial processing finished you are able to  access all your Snowflake resources in Cito and to start your data observability journey by setting up data tests. To receive the latest snapshot of your Snowflake environment just hit the refresh button again.